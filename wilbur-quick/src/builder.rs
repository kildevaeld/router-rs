...
impl<'js> AppBuildCtx<'js> {
    ...
    pub async fn build(
        self,
        ctx: Ctx<'js>,
    ) -> rquickjs::Result<(Class<'js, Router<'js>>, JsRouteContext<'js>)> {
...
pub struct AppBuilder<'js> {
    inits: Vec<Arc<dyn Init + Send + Sync>>,
    builder: Option<rquickjs_modules::Builder>,
    augmentations: BTreeMap<TypeId, Vec<Box<dyn DynAugmentation<JsRouteContext<'js>>>>>,
}

impl<'js> Default for AppBuilder<'js> {
    fn default() -> Self {
        AppBuilder {
...
impl<'js> AppBuilder<'js> {
    ...
    where
        T: Augmentation<JsRouteContext<'js>, V> + Send + Sync + 'static,
        V: 'static,
    {
        let id = TypeId::of::<V>();
