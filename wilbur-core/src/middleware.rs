...
pub trait Middleware<B, C, H>: HSendSync {
...
where
    ...
    C: HSendSync + 'static,
{
    type Handle = BoxHandler<'static, B, C>;
...
pub struct BoxMiddleware<B, C, H> {
    inner: Hrc<dyn Middleware<B, C, H, Handle = BoxHandler<'static, B, C>>>,
}

unsafe impl<B, C, H> Send for BoxMiddleware<B, C, H> where
    Hrc<dyn Middleware<B, C, H, Handle = BoxHandler<'static, B, C>>>: Send
{
}

unsafe impl<B, C, H> Sync for BoxMiddleware<B, C, H> where
    Hrc<dyn Middleware<B, C, H, Handle = BoxHandler<'static, B, C>>>: Sync
{
}
